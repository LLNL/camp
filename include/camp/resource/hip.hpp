  ...
  inline namespace v1
  {
    ...
    namespace
    {
      struct device_guard {
        ...
        {
          noReturnCampHipErrchk(hipGetDevice(&prev_device));
          if (device != prev_device) {
            noReturnCampHipErrchk(hipSetDevice(device));
          } else {
            prev_device = -1;
          }
        }
        ...
        {
          if (prev_device != -1) {
            noReturnCampHipErrchk(hipSetDevice(prev_device));
          }
        }
    ...
    public:
      ...
      void wait() const { noReturnCampHipErrchk(hipEventSynchronize(m_event)); }
    ...
    private:
      ...
      {
        noReturnCampHipErrchk(hipEventCreateWithFlags(&m_event, hipEventDisableTiming));
        noReturnCampHipErrchk(hipEventRecord(m_event, stream));
      }
    };
      ...
      static hipStream_t get_a_stream(int num)
      {
        ...
        std::call_once(m_onceFlag, [] {
          if (streams[0] == nullptr) {
            for (auto &s : streams) {
              noReturnCampHipErrchk(hipStreamCreate(&s));
            }
          }
        });
    ...
    public:
      ...
      {
        if (dev < 0) {
          noReturnCampHipErrchk(hipGetDevice(&dev));
        }
        return Hip(s, dev);
      }
      ...
      void wait()
      {
        ...
        noReturnCampHipErrchk(hipStreamSynchronize(stream));
      }
        ...
        if (size > 0) {
          ...
          switch (ma) {
            ...
            case MemoryAccess::Device:
              noReturnCampHipErrchk(hipMalloc((void**)&ret, sizeof(T) * size));
              break;
            case MemoryAccess::Pinned:
              ...
              noReturnCampHipErrchk(hipHostMalloc((void**)&ret, sizeof(T) * size));
              break;
            case MemoryAccess::Managed:
              noReturnCampHipErrchk(hipMallocManaged((void**)&ret, sizeof(T) * size));
              break;
          }
        }
        return ret;
      ...
      void deallocate(void *p, MemoryAccess ma = MemoryAccess::Unknown)
      {
        ...
        switch (ma) {
          case MemoryAccess::Device:
            noReturnCampHipErrchk(hipFree(p));
            break;
          case MemoryAccess::Pinned:
            // TODO: do a test here for whether managed is *actually* shared
            // so we can use the better performing memory
            noReturnCampHipErrchk(hipHostFree(p));
            break;
          case MemoryAccess::Managed:
            noReturnCampHipErrchk(hipFree(p));
            break;
      ...
      void memset(void *p, int val, size_t size)
      {
        if (size > 0) {
          auto d{device_guard(device)};
          noReturnCampHipErrchk(hipMemsetAsync(p, val, size, stream));
        }
      }
